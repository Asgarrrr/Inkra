...
- `e2e/specs/latex-math.spec.js` fails all four of its scenarios, starting with "opens the seeded document from the sidebar" — the seeded document never opens and the other three cascade from it. Pre-existing and unrelated to the content-search work: verified identical on `HEAD` with that branch's changes stashed and the bundle rebuilt. Note for whoever picks it up — the e2e bundle only builds through `e2e/package.json`'s `build:app` (`--features e2e --bundles app` plus its `--config`); a bare `cargo tauri build` yields an app with no WebDriver plugin, which surfaces as a session-creation timeout rather than as a missing feature, and overwrites the good bundle on the way.
