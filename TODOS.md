...
- Accessibility and CSS lint audit, deferred out of the Biome migration and recorded as decisions in `biome.jsonc`. Four `a11y/*` rules are off (16 findings: 8 `noSvgWithoutTitle`, 5 `noStaticElementInteractions`, 2 `useAriaPropsSupportedByRole`, 1 `useSemanticElements`) and two CSS rules (20 findings: 13 `noImportantStyles`, 7 `noDescendingSpecificity`). This is coverage Oxlint never had, so none of it is a regression — but it is real debt, and turning it on is an audit rather than a toolchain change. They were disabled individually, so new code still gets every other rule and re-enabling is six line deletions.
- Decide what replaces type-aware lint, lost with Oxlint. Biome has no `tsc`-backed engine, so the whole `typeAware: true` rule class is gone — concretely, `no-redundant-type-constituents` on `e2e/wdio.conf.js:17`, flagging a JSDoc union that collapses to `any`, is now unreported. `bun run typecheck` restores `tsc` itself, not type-informed *lint*. Either accept the narrower coverage deliberately or add `typescript-eslint` for that rule class alone.
- Bump both apps' `lib` to `ES2022`. The runtime is a modern WebKit webview and Bun, both fine with it. It lets Biome's `noPrototypeBuiltins` back on (`Object.hasOwn` needs ES2022; the code currently uses the `Object.prototype.hasOwnProperty.call` workaround) and `Array.prototype.at`, which one test had to give up. Small, but it changes a compiler floor, so it wants its own change rather than riding along in someone else's.
...
- Remove Vite+ from the toolchain: [`SPECs/remove-vite-plus-spec.md`](SPECs/remove-vite-plus-spec.md) — three slices (`2a49897`, `7a7d3d1`, `53fa376`). Both apps off the `@voidzero-dev/vite-plus-core` fork onto real Vite 8, which dissolved the `vite` override knot and made Vitest 5 a drop-in; Oxlint + Oxfmt → Biome; then lefthook, `bun run --filter`, `.node-version`, and the removal itself. 162 MB reclaimed, 147 lockfile lines gone, and `bun run ready` exits 0 for the first time since before the migration. Two things worth remembering. A fourth, earlier attempt sent the tests to `bun test` (`7e0c211`) and was reverted: it cost 143 lines of shims and a `--isolate` flag with no home in config, and the dependency it claimed to remove stayed installed anyway — order the slices so the constraint disappears rather than shimming around it. And an incremental `bun install` reports "no changes" while leaving a removed tree on disk; only deleting `node_modules` reclaims it.
- Switch the package manager from pnpm to Bun — workspace definition, catalog, and overrides moved into the root `package.json`; `pnpm-workspace.yaml`, `pnpm-lock.yaml`, and `.npmrc` removed in favour of `bun.lock`. `vp` remains the only entry point and resolves Bun from `packageManager` — superseded by `53fa376`, which removed `vp`; `bun` is now called directly. Dropped without replacement: `catalogMode: prefer` (Bun's `bun add --catalog` is accepted but a no-op as of 1.4, so cataloguing is a manual edit) and `peerDependencyRules`. Left alone deliberately: the `packages/*` and `tools/*` workspace globs match nothing today, and the root `ready` script's `vp run build -r` fails to resolve a `build` task — both predate this change.
