...
- Remove Vite+ from the toolchain: [`SPECs/remove-vite-plus-spec.md`](SPECs/remove-vite-plus-spec.md) — three slices, each leaving the repo green: `vite-plus/test` → Vitest across 57 test files; both apps off the `@voidzero-dev/vite-plus-core` fork onto real Vite 8, which dissolves the `vite` override knot; Oxlint + Oxfmt → Biome at `indentStyle: space` / `lineWidth: 100`, measured to leave only 4 files reformatted; then hooks, task running, and removal. Decided up front: `.node-version` pins Node, and Biome's CSS and a11y findings are deferred rather than folded in. **Slices 1 and 2 landed** (`2a49897`, `7a7d3d1`). An earlier attempt sent the tests to `bun test` (`7e0c211`) and slice 1 reverted it: it cost 143 lines of shims, a `@types/bun` dependency and a `--isolate` flag with no home in config, and the dependency it claimed to remove stayed installed anyway. Real Vite made Vitest 5 a drop-in, so all of it came back out. The lesson to keep: order the slices so the constraint disappears rather than shimming around it.

`tsc` quietly left the fast validation loop when `vp check` did, since Biome does not typecheck. Restored as per-app `typecheck` scripts aggregated by `bun run --filter`, verified to propagate a failing workspace's exit code.

Remaining for slice 3: lefthook replaces `.vite-hooks` and `core.hooksPath`, and should carry `bun run typecheck` as a project-level step — it cannot live in the `staged` block, which appends filenames that make `tsc` ignore the tsconfig. Then `bun run --filter` for task running, `.node-version`, and removing `vite-plus` itself, which is what finally drops the two now-unused forks (`vite-plus-core`, `vite-plus-test`) and the 41 MB native binary, and unblocks `AGENTS.md:110` — still saying `vp check` / `vp test` inside the block `vp config` regenerates on every install. `vp run build -r` in `ready` has been broken since before this work.

Deferred out of slice 2, each recorded as a decision in `biome.jsonc`: re-enable the four `a11y/*` rules (16 findings) and the two CSS rules (20 findings) as a dedicated audit — coverage Oxlint never had, not a regression, and disabled individually so new code still gets every other rule. Bump both apps' `lib` to `ES2022` in its own change, which lets `noPrototypeBuiltins` and `Array.prototype.at` back in (the runtime is a modern WebKit webview and Bun, both fine). One readability nit: `fold/core.ts:56`, `if (!bucket) byName.set(name, (bucket = []))`.

Also lost with Oxlint, and worth a decision rather than a shrug: type-aware lint. Biome has no `tsc`-backed engine, so the whole `typeAware: true` rule class is gone — concretely, the `no-redundant-type-constituents` warning on `e2e/wdio.conf.js:17` is now unreported. `tsc` still typechecks; only type-informed *lint* rules went.
