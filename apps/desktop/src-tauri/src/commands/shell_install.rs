//! Install/uninstall the `inkra` command on the user's PATH.
//!
//! The running Inkra binary is itself the CLI: `main.rs` dispatches to CLI
//! mode when argv[0]'s basename is `inkra`, so "install" is just a symlink
//! from `/usr/local/bin/inkra` to `Inkra.app/Contents/MacOS/Inkra`.
//! When `Inkra.app` is replaced in place by the updater, the symlink still
...
/// Canonical on-disk location for the installed `inkra` shim.
pub const INSTALL_TARGET: &str = "/usr/local/bin/inkra";
...
pub struct CliInstallStatus {
    pub target: String,
    /// Absolute path to the running Inkra binary. Stable while the app is
...
pub enum CliInstallState {
    ...
    Missing,
    /// `target` is a symlink pointing at the current Inkra binary.
    Installed,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::SourceUnknown => write!(
                f,
                "could not determine the Inkra binary path (std::env::current_exe failed)."
            ),
            Self::TargetOccupied(p) => write!(
                f,
                "{} already exists and is not a symlink. Remove it manually if you want Inkra to manage it.",
                p.display()
            ),
...
mod tests {
    ...
    // `Inkra` and `inkra` are the same file).
    ...
    fn classify_installed_when_symlink_matches_source() {
        let dir = tempdir().unwrap();
        let source = make_source(dir.path(), "Inkra");
    ...
    fn classify_stale_when_symlink_points_elsewhere() {
        let dir = tempdir().unwrap();
        let source = make_source(dir.path(), "Inkra");
        let other = make_source(dir.path(), "OldInkra");
        let target = target_path(dir.path());
        fs::create_dir_all(target.parent().unwrap()).unwrap();
    ...
    fn try_symlink_creates_link_and_replaces_stale_one() {
        ...
        let other = make_source(dir.path(), "NewInkra");
